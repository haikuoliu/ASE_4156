...
export function loadSingleEvent(eid, username) {
  return (dispatch, getState) => ( // eslint-disable-line no-unused-vars
    fetchPro(api('events:getSingleEvent', eid, username))
      .then(response => response.json())
      .catch(() => ({ status: 'fail', result: { msg: 'Network Unavailable!' } }))
      .then(json => {
        if (json.status === 'fail') {
          logger.error(api('events:getSingleEvent', eid, username), json.result.msg)
          return
        }
...
export function updateEventContent(username, eid, event) {
  return (dispatch, getState) => { // eslint-disable-line no-unused-vars
    const formData = new FormData()
    formData.append('eid', eid)
