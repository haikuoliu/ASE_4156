...
describe('test get info functions', function() {           
    ...
    it('test getBasicInfo', function(done) {
        ...
        request.get({url:'http://localhost:3000/user/getBasicInfo?username=zehao'}, function optionalCallback(err, res, body) {
            ...
            expect(err).to.equal(null);
            expect(body.status).to.equal("succ");
            expect(body.result.username).to.equal("zehao");
            expect(body.result.birth).to.equal("2017-03-08");
            expect(body.result.gender).to.equal("male");
            expect(body.result.email).to.equal("aa@gmail.com");
            expect(body.result.phone).to.equal("adfasd");
            done();
        });
    });
